...
function freshState() {
  ...
  return { cash: 0, run: 0, shares: 0, rebirths: 0, cp: 0,
    biz: BUSINESSES.map(() => 0), upg: {}, shop: {}, sec: 0, ins: 0, exp: 0,
...
function renderAll(force) {
  ...
  $("#rebirthInfo").textContent = `Company #${S.rebirths + 1} · ${fmt(S.shares)} shares · ${fmt(S.cp)} Country Points`;
  ...
  /* Upgrades */
  renderIf("#upgList", "u" + Object.keys(S.upg).join() + UPGRADES.map(u => aff(u.cost) + (upgAvail(u) ? "a" : "l")).join() + Math.floor(Math.log10(S.run + 1)), () =>
  ...
  $("#rebirthText").textContent = S.shares >= 10 ? `Convert ${fmt(S.shares)} shares into ${cpGain} Country Points (+${cpGain * 10}% revenue forever).` :
    `Reach 10 shares to convert them into Country Points. You have ${fmt(S.shares)}.`;
  $("#rebirthBtn").disabled = cpGain < 1;
  $("#cpBal").textContent = fmt(S.cp) + " CP";
  renderIf("#shop", "sh" + S.cp + Object.keys(S.shop).join(), () =>
    SHOP.map(x => `<div class="card"><div class="info"><b>${x.name}</b><br><span class="muted small">${x.desc}</span></div>
      ${S.shop[x.id] ? `<span class="chip">Owned</span>` : `<button class="btn" data-shop="${x.id}" ${S.cp >= x.cost ? "" : "disabled"}>${x.cost} CP</button>`}</div>`).join(""));
...
function showCountryPicker() {
  ...
  $("#modalBody").innerHTML = `<h3>Pick your home country</h3>
    <p class="muted">Your country sets local flavor and gives a gameplay perk.</p>
    <div id="cpick">${UNIVERSE.map(k => `<div class="card"><div class="info"><b>${COUNTRIES[k].f} ${COUNTRIES[k].n}</b><br>
      <span class="muted small">${COUNTRIES[k].perk ? COUNTRIES[k].perk.name + ": " + COUNTRIES[k].perk.txt : "No special perk"}</span></div>
      <button class="btn" data-country="${k}">Choose</button></div>`).join("")}</div>`;
}
...
function doIPO() {
  ...
  S.shares += p; S.cash = S.shop.warm ? 1e4 : 0; S.run = 0; S.biz = BUSINESSES.map(() => 0); S.upg = {};
  toast(`IPO complete: +${p} shares`); renderAll(true); save();
}
function doRebirth() {
  const gain = Math.floor(S.shares / 10); if (gain < 1) return;
  if (S.settings.confRebirth && !confirm(`Start your next company? You will convert ${fmt(S.shares)} shares into ${gain} Country Points and reset the rest.`)) return;
  S.cp += gain; S.rebirths++; S.shares = 0; S.cash = S.shop.warm ? 1e4 : 0; S.run = 0; S.biz = BUSINESSES.map(() => 0);
  S.upg = {}; S.exp = 0; S.sec = 0; S.ins = 0; S.stocks = freshState().stocks; events = []; temp = [];
  toast(`Company #${S.rebirths + 1} founded. +${gain} Country Points`); renderAll(true); save();
}
...
$("#hustle").addEventListener("click", hustle);
...
$("#hardReset").addEventListener("click", () => {
  if (S.settings.confReset && !confirm("Erase ALL progress, including Country Points?")) return;
